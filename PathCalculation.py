...
##new
def _get_path1(src, dst):
	#Bellman-Ford algorithm
	keys = switches.keys()
	distance = {}
	src_adjacent = []
	previous1 = {}
        log.debug("raja %s",keys)            
	for z in adj[src].keys():
		src_adjacent.append(z) 
		
	m = 0
	c = dst
        while c is not None:
		for e in range(len(src_adjacent)):
			if previous[c] == src_adjacent[e]:
				for f in range(len(keys)):
					log.debug("t %s,srcadjace %s",f,e)
					log.debug("tatata %s,srcadjacent %s",keys[f],src_adjacent[e])
					if keys[f] ==  src_adjacent[e]:
						m = keys[f]
						del keys[f];
						break
	...
	distance[src] = 0
	for i in range(len(keys)-1):
		for u in keys: #nested dict
			#log.debug("uvalue %s",u)      
			for v in adj[u].keys():
	...
	return Path(src, dst, previous1, first_port)  #
##new
def new():
	
	_install_path(prev_path2,matcha)
...
def _delete_path(prev_path, match):
	dst_sw = prev_path.dst
	cur_sw = prev_path.dst
	dst_pck = match.dl_dst
	...
	while cur_sw is not None: #for switch in path.keys():
		...
		log.debug("DELETING forward from switch %s to switch %s output port %s", util.dpid_to_str(cur_sw), util.dpid_to_str(next_sw), adj[cur_sw][next_sw])
		msg.actions.append(of.ofp_action_output(port = adj[cur_sw][next_sw]))
		switches[cur_sw].connection.send(msg)
		next_sw = cur_sw
	...
	def _handle_PacketIn(self, event):
		...
		if packet.effective_ethertype == packet.LLDP_TYPE:
			...
			dst = mac_learning[packet.dst]
                        prev_path = _get_path(self.connection.dpid, dst.dpid)#new
                        #prev_path2 = _get_path1(self.connection.dpid, dst.dpid)#new 
					
			msg = of.ofp_packet_out()
			msg.data = event.ofp.data
			msg.actions.append(of.ofp_action_output(port = dst.port))
			switches[dst.dpid].connection.send(msg)
                        log.debug("Path from %s to %s over path %s", packet.src, packet.dst, prev_path)#new
                        #log.debug("Path2 from %s to %s over path %s", packet.src, packet.dst, prev_path1)#new
			log.debug("Switch %s processed unicast ARP (0x0806) packet, send to recipient by switch %s", self, util.dpid_to_str(dst.dpid))#new
		else:
			...
			dst = mac_learning[packet.dst]
			Switch.prev_path = _get_path(self.connection.dpid, dst.dpid)
			Switch.prev_path2 = _get_path1(self.connection.dpid, dst.dpid)
			if Switch.prev_path is None:
				flood()
				return
			log.debug("Path from %s to %s over path %s", packet.src, packet.dst, Switch.prev_path)
			if self.l3_matching == True: #only match on l2-properties, useful when doing experiments with UDP streams as you can insert a flow using ping and then start sending udp.
				...
				Switch.matcha = match
			_install_path(Switch.prev_path, match)
			...
			self.raiseEvent(NewFlow(Switch.prev_path, match, adj))
...
class _handle_delayevent():
	#_core_name = "delayevent"
	
		
	def abcd(self):
		
		matchaa = Switch.matcha
		prev_path4 = Switch.prev_path
		prev_path3 = Switch.prev_path2	
		log.debug("ddddddddddddddddddddddddddddddddddddD")
		_delete_path(prev_path4, matchaa)
		_install_path(prev_path3, matchaa)
...
def launch (l3_matching=False):
	...
	core.registerNew(_handle_delayevent)	
